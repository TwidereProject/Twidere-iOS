...
class TwitterOAuthPasswordAuthenticator {
    ...
    let consumerKey, consumerSecret: String
    let rest: RestClient
    ...
    func getOAuthAccessToken(_ username: String, password: String) -> Promise<OAuthToken> {
        return firstly { () -> Promise<OAuthToken> in
            let oauth = self.endpoint.getService(auth: OAuthAuthorization(consumerKey, consumerSecret),
                    type: OAuthAPIRestImpl.self)
            return oauth.getRequestToken(oauthCallbackOob)
            ...
            let oauth = self.endpoint.getService(auth: OAuthAuthorization(self.consumerKey, self.consumerSecret,
                    oauthToken: authorizeResponseData.requestToken), type: OAuthAPIRestImpl.self)
            return oauth.getAccessToken(authorizeResponseData.oauthPin)
        }
    }
    ...
    func getAuthorizeRequestData(_ requestToken: OAuthToken) -> Promise<AuthorizeRequestData> {
        ...
        let call = RestCall<AuthorizeRequestData>()
        call.method = .post
        call.path = "/oauth/authorize"
        call.headers = requestHeaders
        call.queries = queries
        call.serializer = DataResponseSerializer { req, resp, data, error in
    ...
    func getAuthorizeResponseData(_ authorizeRequestData: AuthorizeRequestData,
        ...
        let call = RestCall<AuthorizeResponseData>()
        call.method = .post
        call.path = "/oauth/authorize"
        call.headers = requestHeaders
        call.params = forms
    ...
    fileprivate func getVerificationData(_ authorizeResponseData: AuthorizeResponseData,
        ...
        let call = RestCall<AuthorizeRequestData>()
        call.method = .post
        call.path = "/account/login_verification"
        call.headers = requestHeaders
        call.params = forms
        call.serializer = DataResponseSerializer { req, resp, data, error in
            let result = AuthorizeRequestData.parseFromHttpResult(data!)
